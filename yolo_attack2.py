...
class PatchTrainer(object):
    def __init__(self):
        ...
        self.max_extractor = DetMaxExtractor().cuda()
    ...
    def init_logger(self):
        ...
        detail_info = 'YOLOv3，没有加nms，损失函数只有det loss+tv loss，对梯度进行了截断处理，增大了贴片的大小，将det loss的单张图片的loss还是max, 去除梯度截断'
    ...
    def train(self):
        ...
        for epoch in range(10000):
            ...
            for image_batch, clothes_boxes_batch, people_boxes_batch, labels_batch, landmarks_batch, segmentations_batch in tqdm(
                ...
                det_loss, det_union_loss = self.max_extractor(self.model_, p_img_batch)
                ...
                logging.info(f'epoch: {epoch} iter: {i_batch} |det_union_loss: {det_union_loss}')
                # logging.info(f'conf_loss_union_image: {conf_loss_union_image}')
                logging.info(f"epoch: {epoch} iter: {i_batch} |union_attack_loss: {union_attack_loss}")

                # Dynamic update weight parameter
                with torch.no_grad():
                    if last_det_union != -1:
                        r_uniou_det = det_union_loss / last_det_union
                        ...
                        rs = torch.stack([r_det_loss, r_tv, r_union, r_uniou_det])
                        rs = torch.exp(rs)
                        sum_rs = torch.sum(rs)
                        w_det_union = (N * rs[-1]) / sum_rs
                        # w_conf_union = N * rs[1] / sum_rs
                        w_det = N * rs[0] / sum_rs
                        w_tv = N * rs[1] / sum_rs
                        w_union_attack = N * rs[2] / sum_rs

                # loss = w_det * det_loss + w_tv * tv_loss
                # loss = det_loss + tv_loss * 2.5
                loss = w_det_union * det_union_loss + w_tv * tv_loss + w_union_attack * union_attack_loss + w_det * det_loss
                # loss = det_union_loss
                ...
                # update last loss
                last_det_union = det_union_loss.detach()
                ...
                ep_iou_all += det_union_loss.detach().cpu().numpy()
                ...
                if i_batch % 23 == 0:
                    ...
                    self.writer.add_scalar('loss/det_union_loss', det_union_loss.detach().cpu().numpy(),
                                           iteration)
                ...
                del adv_batch_t, p_img_batch, adv_batch_mask_t, det_loss, tv_loss, loss, det_union_loss, union_attack_loss
            ...
            # del max_prob, det_loss, p_img_batch, tv_loss, loss, adv_batch_mask_t, det_union_loss, union_iou_loss
            torch.cuda.empty_cache()
